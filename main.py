    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            if "acecwply" in url:
                ...
                url = f"https://radharadhe.vercel.app/ITsGOLU_OFFICIAL?url={url}@bots_updatee&user_id={user_id}"
                url = f"https://radharadhe.vercel.app/ITsGOLU_OFFICIAL?url={url}&token={cptoken}&auth=@scammer_botxz1"
                mpd, keys = helper.get_mps_and_keys(url)
                url = mpd
                keys_string = " ".join([f"--key {key}" for key in keys])

            elif "classplusapp" in url:
                signed_api = f"https://radharadhe.vercel.app/ITsGOLU_OFFICIAL?url={url}@bots_updatee&user_id={user_id}"
