    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            if "acecwply" in url:
                ...
                #url = f"https://cp-auto-api-795428f14173.herokuapp.com/resolve?url={url}@bots_updatee&user_id={user_id}"
                url = f"https://cp-auto-api-795428f14173.herokuapp.com/resolve?url={url}&token={cptoken}&auth=@scammer_botxz1"
                mpd, keys = helper.get_mps_and_keys(url)
                url = mpd
                keys_string = " ".join([f"--key {key}" for key in keys])

            #elif "classplusapp" in url:
                #signed_api = f"https://cp-auto-api-795428f14173.herokuapp.com/resolve?url={url}@bots_updatee&user_id={user_id}"
